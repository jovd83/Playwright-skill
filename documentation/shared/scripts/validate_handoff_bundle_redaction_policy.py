...
from export_handoff_bundle import (  # type: ignore
    ...
    compile_extra_regexes,
    load_redaction_policy_file,
)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Validate a handoff bundle redaction policy JSON file."
    )
    ...
    parser.add_argument("--policy", help=f"Explicit redaction policy path. Defaults to <docs-root>/{DEFAULT_REDACTION_POLICY_FILE_NAME}.")
...
def main() -> int:
    ...
    policy_path = Path(args.policy).resolve() if args.policy else (docs_root / DEFAULT_REDACTION_POLICY_FILE_NAME).resolve()

    try:
        policy = load_redaction_policy_file(policy_path)
        compile_extra_regexes(list(policy["extra_redaction_regexes"]))
