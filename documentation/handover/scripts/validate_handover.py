...
def extract_pointer(sections: dict[str, str]) -> str | None:
    pointer = normalize_value(first_meaningful_line(sections["Session-state pointer"]))
...
def validate_document(path: Path) -> list[str]:
    ...
    if section_order != REQUIRED_SECTIONS:
        errors.append("Section order does not match the required handover template order.")
    ...
    if status == "done":
        ...
        errors.append("Non-done handover must identify the next owner.")

    pointer = extract_pointer(sections)
    if status != "done" and pointer is None:
        errors.append("Non-done handover must include a session-state pointer.")

    if pointer is not None:
        if pointer_uses_backslashes(pointer):
            errors.append("Session-state pointer must use forward slashes.")
        linked_path = resolve_pointer(path, pointer)
        if not linked_path.exists():
            errors.append(f"Session-state pointer does not exist: {pointer}")
        else:
            try:
                linked_status = extract_status_from_linked_file(linked_path, "# Session State")
            except ValueError as exc:
                errors.append(str(exc))
            else:
                if linked_status != status:
                    errors.append(
                        f"Status mismatch between handover ({status}) and linked session-state ({linked_status})."
                    )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Validate a handover markdown file.")
    parser.add_argument("path", help="Path to the handover markdown file.")
