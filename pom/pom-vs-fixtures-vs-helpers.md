...
### Helpers as State Mutators

```typescript
// BAD: helper function handles login or browser state
...
**Problem:** Helpers should be stateless. Mutating the browser or managing sessions inside helpers creates untraceable side effects and makes tests harder to parallelize.

**Fix:** Move all authentication and setup code into **Fixtures** (`test.extend`). Move all UI interaction code into **Page Object Models**. Use helpers *only* for stateless utilities like `generateRandomEmail()`. Never pass the `Page` object to a helper.
