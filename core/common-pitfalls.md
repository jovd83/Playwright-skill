...
| 21 | Skipping confirmation assertions | Always assert explicit "Success" or "Deleted" messages |

## Pitfall 21: Verifying Side Effects but Skipping User Feedback

**Symptom**: State changes correctly, but the UI feedback is broken (e.g., no success message). Users think the action failed.

**Why it happens**: Developers focus on the "Gold Source" of truth (database change, redirect, item removed from list) but forget the user's perception of the action.

**Fix**: Always assert the explicit success message/toast alongside the side effect.

**TypeScript**
```typescript
import { test, expect } from '@playwright/test';

// BAD — verifies item is gone, but ignores user feedback
test('bad: only verifies side effect', async ({ page }) => {
  await page.goto('/settings');
  await page.getByRole('button', { name: 'Delete Account' }).click();
  await page.getByRole('button', { name: 'Confirm' }).click();

  await expect(page).toHaveURL('/'); // Side effect met: redirected
  // Problem: If the "Account Deleted" toast is broken, the user sees home but doesn't know why.
});

// GOOD — verifies both
test('good: verifies feedback and side effect', async ({ page }) => {
  await page.goto('/settings');
  await page.getByRole('button', { name: 'Delete Account' }).click();
  await page.getByRole('button', { name: 'Confirm' }).click();
...
## Pitfall 21: Using Placeholder Assertions (`toBeDefined`, `toBeTruthy`)

**Symptom**: All tests pass, but they don't actually verify anything. The test claims 100% success despite major UI regressions.

**Why it happens**: Developers (or AI agents) write "scaffold" tests to satisfy a file-per-scenario requirement without doing the work of identifying stable locators and assertions.

**Fix**: Always assert on a visible requirement or state. Never use `expect(page).toBeDefined()` as a substitute for an actual UI check.

**TypeScript**
```typescript
// BAD — placeholder test with no real validation
test('user can log in', async ({ page }) => {
  await page.goto('/login');
  // ... no interaction ...
  expect(page).toBeDefined(); // This will ALWAYS pass
});

// GOOD — web-first assertion on actual requirement
