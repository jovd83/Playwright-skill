...
| State-changing action success | `expect(page.getByText(/success|deleted/i)).toBeVisible()` | Verifies explicit user feedback |

## Patterns

### Action Confirmation (Toasts & Success Messages)

**Use when**: Verifying that a state-changing action (delete, save, update) gave the user explicit feedback.
**Avoid when**: The action is only a read-only navigation.

Testing side effects (like an item disappearing) is necessary, but testing the confirmation message is what ensures the user's experience is intact.

**TypeScript**
```typescript
import { test, expect } from '@playwright/test';

test('verify success message after deletion', async ({ page }) => {
  ...
  // 1. Assert explicit user feedback (toast/success message)
  // Use regex to be resilient to minor text variations
  await expect(page.getByText(/item deleted|successfully removed/i)).toBeVisible();

  // 2. Assert side effect (item is gone)
  await expect(page.getByTestId('item-row')).not.toBeVisible();
});
```
...
**Tip**: Toasts often disappear quickly. If your test misses them, use `page.pause()` or trace viewer to verify the locator.
